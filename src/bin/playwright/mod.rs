...
fn run_shared_playwright_script(
    ...
    let result = client.run_playwright(source, allow_close)?;
    render_shared_result(&result)
}
...
mod tests {
    ...
    #[test]
    fn shared_results_render_as_pretty_json() {
        let result = render_shared_result(&json!({
            "ok": true,
            "mode": "playwright-crx"
        }))
        .unwrap();
        assert!(result.contains("\"mode\": \"playwright-crx\""));
    }
}
